...
def normal_plot(mu, dp, n, x_=None, abaixo=None, acima=None, intervalo = None ):
    ...
    plot_X, = plt.plot(x_axis, y)
    plot_X.set_label('Distribuição de $X$')
    ...
    if abaixo is not None:
        ...
        plot_fill.set_label("Região abaixo de {:.1f}".format(abaixo))
        
    if acima is not None:
        plot_fill = plt.fill_between(x_axis, y1=y_amostral, where=(x_axis>acima))
        plot_fill.set_label("Região acima de {:.1f}".format(acima))
    ...
    if x_ is not None:
        ...
        plot_x_.set_label('Linha da amostra $\overline{x}$')
